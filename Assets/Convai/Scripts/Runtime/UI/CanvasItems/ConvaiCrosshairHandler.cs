       ...
       private string FindInteractableReference(GameObject hitGameObject)
{
    ...
    foreach (KeyValuePair<GameObject, string> kvp in _interactableReferences)
    {
        if (kvp.Key == null) {
            Debug.LogWarning("Null key found in _interactableReferences dictionary.");
            continue; // skip null keys
        }
        // Compare transforms safely
        Transform hitTransform = hitGameObject.GetComponentInParent<Transform>();
        if (hitTransform != null && hitTransform == kvp.Key.transform)
            return kvp.Value;
    }
